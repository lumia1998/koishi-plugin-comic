...
export const Config = Schema.object({
  ...
  defaultSource: Schema.union(['jm', 'bika'])
    .description('默认漫画源')
    .default('jm'),
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.command('comic detail <source:string> <id:string>', '查看漫画详情和章节列表')
    .action(async ({ session }, source, id) => {
      if (!source || !id) return '用法: comic detail <jm|bika> <ID>'
      source = source.toLowerCase()
      if (source !== 'jm' && source !== 'bika') return 'source 必须是 jm 或 bika'
  ...
  ctx.command('comic download <source:string> <id:string> [chapterId:string]', '下载漫画PDF（默认第一话）')
    .action(async ({ session }, source, id, chapterId) => {
      if (!source || !id) return '用法: comic download <jm|bika> <ID> [章节ID]'
      source = source.toLowerCase()
      if (source !== 'jm' && source !== 'bika') return 'source 必须是 jm 或 bika'
      ...
      try {
        ...
        const fileElement = h.file(buffer, 'application/pdf', { filename })
  ...
  ctx.command('comic leaderboard [source:string] [mode:string]', '查看排行榜')
    .action(async ({ session }, source, mode) => {
      ...
      try {
        const result = await apiGet<ApiResponse<ComicItem[]>>(`/api/${source}/leaderboard`, { mode })
        const srcLabel = source === 'jm' ? '禁漫' : '哔咔'
        const modeMap: Record<string, string> = { day: '日榜', week: '周榜', month: '月榜', total: '总榜' }

        if (!result.data?.length) return `${srcLabel} ${modeMap[mode]}暂无数据`
  ...
  ctx.command('comic category <source:string> <name:string>', '按分类浏览漫画')
    .action(async ({ session }, source, name) => {
      if (!source || !name) return '用法: comic category <jm|bika> <分类名>'
      source = source.toLowerCase()
      if (source !== 'jm' && source !== 'bika') return 'source 必须是 jm 或 bika'

      try {
        const result = await apiGet<ApiResponse<ComicItem[]>>(`/api/${source}/category`, { name })
        const srcLabel = source === 'jm' ? '禁漫' : '哔咔'

        if (!result.data?.length) return `${srcLabel} 分类「${name}」暂无数据`
  ...
  ctx.command('comic latest [source:string]', '查看最近更新')
    .action(async ({ session }, source) => {
      ...
      try {
        const result = await apiGet<ApiResponse<ComicItem[]>>(`/api/${source}/latest`)
        const srcLabel = source === 'jm' ? '禁漫' : '哔咔'

        if (!result.data?.length) return `${srcLabel} 最近更新暂无数据`
  ...
  ctx.command('comic random [source:string]', '随机推荐漫画')
    .action(async ({ session }, source) => {
      ...
      try {
        ...
        if (!comic?.title) return `${srcLabel} 随机推荐暂无数据`
