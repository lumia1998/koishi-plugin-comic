...
function apply(ctx, config) {
    ...
    ctx.command('comic category <name:string>', '按分类浏览漫画')
        ...
        if (!name)
            return '用法: comic category <分类名>';
        try {
            const [jmResult, bikaResult] = await Promise.all([
                apiGet('/api/jm/category', { name }).catch(() => null),
                apiGet('/api/bika/category', { name }).catch(() => null),
            ]);
            const parts = [];
    ...
    ctx.command('comic random', '随机推荐漫画')
        ...
        try {
            const [jmResult, bikaResult] = await Promise.all([
                apiGet('/api/jm/random').catch(() => null),
                apiGet('/api/bika/random').catch(() => null),
            ]);
            const parts = [];
            if (jmResult && jmResult.success !== false && jmResult.data?.length) {
